...
class B4URLTests(TestCase):
    ...
    def test_url_b4_plan_delete(self):
        url = "b4:plan_delete"
        obj = models.Plan.objects.last()
        # obj = models.Plan.objects.create(user=self.custom_variables.get('user'), name="for delete")
        self.assertEqual(bool(obj), True)
        pk = obj.pk
        response = self.custom_client.get(reverse(url, kwargs={"pk": pk}))
        self.assertEqual(response.status_code, 200)
    ...
    def test_url_b4_task_delete(self):
        url = "b4:task_delete"
        obj = models.Task.objects.last()
        # obj = models.Task.objects.create(user=self.custom_variables.get('user'), name="for delete")
        self.assertEqual(bool(obj), True)
        pk = obj.pk
        response = self.custom_client.get(reverse(url, kwargs={"pk": pk}))
        self.assertEqual(response.status_code, 200)
